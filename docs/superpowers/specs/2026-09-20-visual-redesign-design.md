...
- dark: `--win-bg: var(--bg-body)`, title bar uses `--border-pink` background with `--bg` text, `--dither-ink` a faint tint of `--border-pink`. All other colours unchanged.
...
3. Blob layer: a fixed pseudo-element filled with `--dither-ink`, using an inline SVG `feTurbulence` plus `feComponentTransfer` (discrete threshold) as `mask-image`, with `image-rendering: pixelated` and the mask scaled so each noise cell is 3 to 4 px. Blobs sit behind the windows and stay low-contrast in dark and light, full-contrast in pink.
...
- new: `assets/css/spacing.css`, `window.css`, `button.css`, `background.css`, `static/fonts/DepartureMono-Regular.woff2` (+ license)
- edited: `colours.css`, `layout.css`, `header.css`, `typography.css`, `side_image.css`, `social.css`, `boot.css`, `crt.css`, `head.html` (bundle list, fonts, script), `baseof.html`, `header.html`, `title.html`, `theme_init.html`, `side_*.html`, `assets/js/theme.js`
